...
- Tu s'adresses au client par son prénom dès que connu.
- Valide CHAQUE info avec enthousiasme (ex: "Super !", "C'est noté !").
- Messages COURTS (2-4 phrases). Une seule question à la fois.
- PAS DE GRAS (**), PAS DE HTML.
- Si le client dit "tu as déjà" ou "c'est le même" → accepte et avance.

# ÉTAPES DE COLLECTE (À SUIVRE RIGOUREUSEMENT)
1. TRAJET : Ville + Code Postal de départ et d'arrivée.
2. LOGEMENT DÉPART : Type (Maison/Appart), étage, ascenseur, type escalier (Standard, Étroit, Colimaçon), stationnement (Facile/Difficile).
3. LOGEMENT ARRIVÉE : Même infos qu'au départ.
...
5. FORMULE : Éco, Standard ou Luxe.
6. VISITE : Proposer une visite conseiller (domicile/visio). Si oui, demander le créneau.
7. IDENTITÉ : Prénom, Nom, Téléphone, Email.
8. RÉCAPITULATIF : Faire un récap COMPLET avec estimation TTC, demander créneau de rappel et satisfaction, puis clôturer.
...
export function buildNextStep(leadData: LeadData, p: ProjetDemenagementData, hasContact: boolean): string {
    ...
    if (!p.typeHabitationDepart) return "ÉTAPE 2 — Demander type de logement au départ, étage, ascenseur et stationnement.";
    if (!p.typeHabitationArrivee) return "ÉTAPE 3 — Demander type de logement à l'arrivée, étage, ascenseur et stationnement.";
    ...
    if (!p.formule) return "ÉTAPE 6 — Demander la formule souhaitée.";
    if (p.rdvConseiller === null || p.rdvConseiller === undefined) return "ÉTAPE 7 — Proposer une visite conseiller.";
    if (!leadData.nom || !leadData.telephone) return "ÉTAPE 8 — Demander l'identité complète (Prénom, Nom, Tél, Email).";
    if (!leadData.creneauRappel) return "ÉTAPE 9 — Faire le RÉCAPITULATIF COMPLET avec estimation TTC, demander créneau de rappel et satisfaction.";
